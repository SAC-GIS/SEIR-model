...
class Train_Dynamic_SEIR:
    ...
    def __init__(self, data: pandas.core.frame.DataFrame,
        ...
        self.I_pre = []
        self.R_pre = []
        # count the number of days before the first traning point
        self.past_days = data['Days'].min()

        # other parameters within SEIR model
        self.c = c  # intial guess
        self.b = b  # intial guess
        self.alpha = alpha  # intial guess
        self.rateSI = self._calculate_beta(
            c=self.c, t=0, b=self.b, alpha=self.alpha)  # intial guess
        self.rateIR = rateIR
        self.rateAl = rateAl
    ...
    def _update(self):
        ...
        for t in range(0, self.steps):
            ...
            loss_to_beta = -2*(self.Infected[t] - self.I_pre[t])*(
                self.I_pre[t])*t*self.Susceptible[t]/self.numIndividuals

            # use chain rule to calculate partial derivatives
            beta_to_alpha = -self.c*formula * \
                (t+self.b)*(formula-1)*pow((1+formula), -3)
            beta_to_b = -self.c*formula*self.alpha * \
                (formula - 1)*pow((1+formula), -3)
        ...
        for e in range(self.epoch):
            ...
            for t in range(0, self.steps):
                if t == 0:
                    ...
                    S_to_E = (
                        self.rateSI * self.Susceptible[t] * self.Infected[t]) / self.numIndividuals
    ...
    def plot_fitted_beta_R0(self, real_obs: pandas.core.frame.DataFrame):
        ...
        Rlist = [x / self.rateIR for x in self.betalist]
        plt.plot(self.estimation['Time'], Rlist, color='blue')

        # set x tricks
        datemin = real_obs['date'].min()
        numdays = len(real_obs)
        labels = list((datemin + datetime.timedelta(days=x)
                       ).strftime('%m-%d') for x in range(numdays))
        plt.xticks(list(range(numdays)), labels, rotation=90, fontsize=10)
        plt.xlabel('2021 Date')
        plt.ylabel('Rate')
        plt.title(
            'Fitted Dynamic Contact Rate and Transmissibility of COVID-19 over time', fontsize=20)
    ...
    def plot_fitted_result(self, real_obs: pandas.core.frame.DataFrame):
        fig, ax = plt.subplots(figsize=(15, 6))
        plt.plot(
            self.estimation['Time'], self.estimation['Estimated_Infected'], color='green')
        plt.plot(self.estimation['Time'], real_obs['I'], color='y')
        plt.plot(self.estimation['Time'],
                 self.estimation['Estimated_Exposed'], color='blue')
...
class dynamic_SEIR:
    ...
    def __init__(self, eons=1000, Susceptible=950, Exposed=100, Infected=50, Resistant=0, rateIR=0.01, rateAl=0.1,
        ...
        self.rateIR = rateIR
        self.rateAl = rateAl
        self.numIndividuals = Susceptible + Infected + \
            Resistant + Exposed  # total population
        self.alpha = alpha
        self.c = c
        self.b = b
    ...
    def run(self, death_rate):
        ...
        for i in range(1, self.eons):  # number of prediction days
            ...
            # print(self.rateSI)
            S_to_E = (self.rateSI *
                      Susceptible[-1] * Infected[-1]) / self.numIndividuals
    ...
    def plot(self, title, ylabel, xlabel, starting_point):
        ...
        plt.plot(self.results['Time'],
                 self.results['Resistant'], color='palegreen')
        ...
        plt.xlabel(xlabel)
        plt.ylabel(ylabel)
        plt.legend(['Susceptible', 'Infected', 'Exposed', 'Removed', 'Heal', 'Death'], prop={
                   'size': 12}, bbox_to_anchor=(0.5, 1.02), ncol=6, fancybox=True, shadow=True)
    ...
    def plot_noSuscep(self, title, ylabel, xlabel, starting_point):
        ...
        plt.xlabel(xlabel)
        plt.ylabel(ylabel)
        plt.legend(['Infected', 'Removed', 'Exposed', 'Heal', 'Death'], prop={
                   'size': 12}, bbox_to_anchor=(0.5, 1.02), ncol=5, fancybox=True, shadow=True)
        plt.title(title, fontsize=20)
        plt.show()
...
def plot_test_data_with_MAPE(test, predict_data, title):
    ...
    # Draw plot
    fig, ax = plt.subplots(figsize=(15, 6))
    plt.plot(test['date'], y, color='black')
    plt.plot(test['date'], y_pred, 'k--')

    plt.xlabel('2021 Shanghai Date')
